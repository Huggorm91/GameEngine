...
bool GraphicsEngine::Initialize(HWND windowHandle, bool enableDeviceDebug)
{
	...
	try
	{
		...
		// Material
		{
			myDefaultMaterial.SetAlbedoTexture(&myTextures.DefaultFXTexture);
			myDefaultMaterial.SetNormalTexture(&myTextures.DefaultNormalTexture);
...
{
	// Albedo
	if (!RHI::LoadTexture(&myTextures.DefaultAlbedoTexture, Helpers::string_cast<std::wstring>(someSettings.DefaultAlbedoTexture)))
	{
