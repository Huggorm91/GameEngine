...
namespace Crimson
{
    // Just a chunk of memory. Does not type-check anything! So user has to ensure they are using valid types.
    class MemoryBlock
    {
    public:
        MemoryBlock();
        ...
        std::pair<size_t, bool> Allocate(const size_t& aSize);

        std::pair<size_t, bool> AddValue(const void* aValue, const size_t& aSize);
        template<typename value>
        std::pair<size_t, bool> AddValue(const value& aValue);
        template<typename value>
        std::pair<size_t, bool> AddValue(value&& aValue);
    ...
    template<typename value>
    inline std::pair<size_t, bool> MemoryBlock::AddValue(const value& aValue)
    {
    ...
    template<typename value>
    inline std::pair<size_t, bool> MemoryBlock::AddValue(value&& aValue)
    {
        bool hasResized = false;
        size_t valueSize = sizeof(value);
        if (myCurrentSize < myCurrentIndex + valueSize)
