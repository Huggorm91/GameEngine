...
class ExampleComponent : public Component
{
	...
	// 4. Add 'case ComponentType::Example: { break; }' to switch in LoadComponent() in ComponentType.cpp
...
public:
	...
	// ExampleComponent(const Json::Value& aJson, GameObject* aParent);
	~ExampleComponent() = default;

	// ExampleComponent& operator=(const ExampleComponent& aComponent);

	// void Init(const Json::Value& aJson, GameObject* aParent) override;
